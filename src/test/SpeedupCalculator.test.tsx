...
describe('SpeedupCalculator Component', () => {
    ...
    it('renders the speedup calculator with input field', () => {
        ...
        expect(screen.getByText(new RegExp(`${t(T.missionsGrid.speedupCalculator.title)}`, 'i'))).toBeInTheDocument();
    ...
    it('does not show calculated result when input is empty', () => {
        render(<SpeedupCalculator />);
        expect(screen.queryByText(new RegExp(t(T.missionsGrid.speedupCalculator.calculatedAddition), 'i'))).not.toBeInTheDocument();
    });
    ...
    it('displays calculated tokens for valid time input (days + time)', () => {
        ...
        expect(screen.getByText(t(T.missionsGrid.speedupCalculator.calculatedAddition))).toBeInTheDocument();
    ...
    it('displays calculated tokens for complex time input', () => {
        ...
        expect(screen.getByText(new RegExp(`\\+832`, 'i'))).toBeInTheDocument();
    });

    it('shows link to external training calculator', () => {
        render(<SpeedupCalculator />);
        const link = screen.getByText(new RegExp(`${t(T.missionsGrid.speedupCalculator.title)}`, 'i')).closest('a');
