...
describe('SpeedupInputs Component', () => {
        ...
        screen.getByText(
          t(
            translations.common.speedupCategories[
              cat as keyof typeof translations.common.speedupCategories
            ],
          ),
        ),
      ).toBeInTheDocument()
  ...
  it('displays speedup volume label and conversion rate', () => {
    render(<SpeedupInputs />)
    expect(
      screen.getByText(t(translations.missionsGrid.speedupCalculator.manualTitle)),
    ).toBeInTheDocument()
    expect(
      screen.getByText(t(translations.missionsGrid.speedupCalculator.ratio)),
    ).toBeInTheDocument()
  })
  ...
  it('displays total packaged minutes correctly', () => {
    ...
    expect(
      screen.getByText(t(translations.missionsGrid.speedupCalculator.totalPackaged)),
    ).toBeInTheDocument()
  })
  ...
  it('does not show volume yield when total is zero', () => {
    render(<SpeedupInputs />)
    expect(
      screen.queryByText(t(translations.missionsGrid.speedupCalculator.yield)),
  ...
  it('displays correct token calculation for 480 minutes', () => {
    ...
    expect(
      screen.getByText(new RegExp(`\\+2 ${t(translations.commandersList.tokens)}`, 'i')),
    ).toBeInTheDocument()
  })
  ...
  it('includes calculated speedup time in token calculation', () => {
    ...
    expect(
      screen.getByText(new RegExp(`\\+8 ${t(translations.commandersList.tokens)}`, 'i')),
    ).toBeInTheDocument()
  })
})
