    ...
    return (
        <div className="space-y-3 pt-2">
            <div className="flex justify-between items-center px-1">
                <Label className="text-[10px] font-black text-neutral-400 uppercase tracking-widest">{t(T.missionsGrid.speedupCalculator.manualTitle)}</Label>
                <span className="text-[10px] font-bold text-blue-500/60 uppercase">{t(T.missionsGrid.speedupCalculator.ratio)}</span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                {Object.entries(speedupMinutes).map(([cat, val]) => (
                    <div key={cat} className="space-y-1">
                        <Label className="text-[9px] font-bold text-neutral-500 uppercase pl-1">{t(T.common.speedupCategories[cat as keyof typeof T.common.speedupCategories])}</Label>
                        <Input
                            type="text"
                ...
                <div className="col-span-1 p-2 rounded-lg bg-white/5 border border-white/10 flex flex-col justify-center items-center">
                    <span className="text-[8px] text-neutral-500 font-bold uppercase">{t(T.missionsGrid.speedupCalculator.totalPackaged)}</span>
            ...
            {totalSpeedupTokens > 0 && (
                <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-500/20 flex justify-between items-center animate-in fade-in duration-300">
                    <span className="text-[10px] text-neutral-400 font-bold uppercase tracking-tighter">{t(T.missionsGrid.speedupCalculator.yield)}</span>
                    <span className="text-sm font-black text-blue-400">+{totalSpeedupTokens} {t(T.commandersList.tokens)}</span>
                </div>
            )}
        </div>
    );
