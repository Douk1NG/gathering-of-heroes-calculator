    ...
    return (
        <div className="space-y-2">
            <div className="flex justify-between items-center px-1">
                <Label className="text-[10px] font-black text-neutral-400 uppercase tracking-widest">{t(T.missionsGrid.gemsCalculator.title)}</Label>
                <span className="text-[10px] font-bold text-yellow-500/60 uppercase">{t(T.missionsGrid.gemsCalculator.ratio)}</span>
            </div>
            ...
            {gemSpendTokens > 0 && (
                <div className="p-3 rounded-lg bg-black/20 border border-white/5 flex justify-between items-center">
                    <span className="text-[10px] text-neutral-500 font-bold uppercase tracking-tighter">{t(T.missionsGrid.gemsCalculator.yield)}</span>
                    <span className="text-sm font-black text-yellow-500">+{gemSpendTokens} {t(T.commandersList.tokens)}</span>
                </div>
            )}
        </div>
    );
