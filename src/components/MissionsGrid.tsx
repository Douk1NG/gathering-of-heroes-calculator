...
export function MissionsGrid() {
    ...
    const challengeTotal = CHALLENGE_MISSIONS.reduce((acc, m) => {
        return acc + (missions.challenge[m.id] ? m.tokens : 0);
    }, 0);

    // Derived Yields
    const gemSpendTokens = Math.floor(missions.totalGemsSpent / 2000) * 30;
    const manualSpeedupMinutes = Object.values(missions.speedupMinutes).reduce((a, b) => a + b, 0);
    const calcMinutes = getSpeedupMinutes();
    const totalSpeedupTokens = Math.floor((manualSpeedupMinutes + calcMinutes) / 480) * 2;

    const getRepeatableTotal = () => {
        return gemSpendTokens + totalSpeedupTokens;
    };
                ...
                <CardContent className="space-y-6 pt-6 flex-1">
                    ...
                    <div className="space-y-3 pt-2">
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                            {Object.entries(missions.speedupMinutes).map(([cat, val]) => (
                    ...
                    <div className="mt-auto pt-4 border-t border-white/5 flex justify-between items-center">
                        ...
                        <span className="text-xl font-black text-red-500">{getRepeatableTotal()} Tokens</span>
                    </div>
                </CardContent>
            </Card>
        </div>
